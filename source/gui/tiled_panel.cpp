    ...
    m_type{ type }, m_tile_size_reference{tile_size_reference}
{
    ...
    add_rule(Property::SIZE, [this](Layout& layout) {
        m_cached_tile_size = m_tile_size_reference->layout.height;
...
void TiledPanel::draw(const glm::vec2& position_transform, sf::RenderTarget& window) {
    ...
    m_tile.setScale(m_cached_tile_size / 16, m_cached_tile_size / 16);
        for (size_t x = 0; x < m_cached_tiles_count.x; ++x) for (size_t y = 0; y < m_cached_tiles_count.y; ++y) {
            if (m_type == Type::Paper) {
                ...
                else if ((x % 4 != 0) && y == m_cached_tiles_count.y - 1)
            ...
            m_tile.setPosition(
                static_cast<int>(position_transform.x + layout.x + x * m_cached_tile_size),
                static_cast<int>(position_transform.y + layout.y + y * m_cached_tile_size)
            );
            window.draw(m_tile);
        }
}
